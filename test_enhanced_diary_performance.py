...
class EnhancedDiaryRecommendationTest:
    def __init__(self):
    ...
    def create_mock_user_with_multiple_interests(self, user_id, num_interests=5):
        """创建具有多种兴趣的模拟用户"""
        all_interests = ['自然风光', '历史文化', '美食', '购物', '娱乐', '展馆展览', '运动健身', '教育培训']
        selected_interests = random.sample(all_interests, min(num_interests, len(all_interests)))
        
        test_user = {
            'id': user_id,
            'name': f'test_user_{user_id}',
            'likes_type': selected_interests,
            'password': 'test123',
                ...
                for run in range(3):  # 每个用户运行3次取平均值
                    ...
                    try:
                        traditional_result, traditional_time, traditional_memory, traditional_ops = \
                            self.measure_detailed_performance(
                                self.user_manager.getRecommendDiariesTraditional, user_id, topK
                            )
                    ...
                    try:
                        optimized_result, optimized_time, optimized_memory, optimized_ops = \
                            self.measure_detailed_performance(
                                self.user_manager.getRecommendDiaries, user_id, topK
                            )
    ...
    def create_comprehensive_charts(self):
        ...
        ax1.set_title('日记推荐算法执行时间对比', fontsize=14, fontweight='bold')
        ax1.legend(fontsize=12)
        ax1.grid(True, alpha=0.3)
        ax1.set_yscale('log')  # 使用对数坐标更好地显示差异
        ...
        ax2.set_title('日记推荐算法内存使用对比', fontsize=14, fontweight='bold')
        ...
        plt.suptitle('日记推荐算法全面性能对比分析', fontsize=18, fontweight='bold', y=0.98)
        
        # 保存图表
        plt.savefig('comprehensive_diary_recommendation_analysis.png', dpi=300, bbox_inches='tight')
        print("\n全面性能分析图表已保存为: comprehensive_diary_recommendation_analysis.png")
    ...
    def generate_detailed_report(self):
        ...
        print(f"\n🔍 算法复杂度分析:")
        print(f"   传统算法复杂度: O(T × S × D + N log k)")
        print(f"     其中: T=用户喜好类型数, S=景点数, D=每个景点的日记数")
        print(f"           N=总日记数, k=归并路数")
        print(f"     实际表现: ≈ O(N log k)")
        
        print(f"\n   优化算法复杂度: O(N log N)")
        print(f"     通过IndexHeap实现更高效的归并排序")
        
        print(f"\n🚀 主要优化策略:")
        print(f"   1. 数据结构优化:")
        print(f"      • 使用IndexHeap替代传统归并排序")
        print(f"      • 减少重复数据遍历和拷贝")
        
        print(f"   2. 算法流程优化:")
        print(f"      • 统一的迭代器模式处理不同类型数据")
        print(f"      • 更高效的去重机制")
        ...
        print(f"\n📈 性能提升分析:")
        best_speedup = max([self.results['traditional']['times'][i] / self.results['optimized']['times'][i] 
                           for i in range(len(self.test_sizes)) if self.results['optimized']['times'][i] > 0])
        worst_speedup = min([self.results['traditional']['times'][i] / self.results['optimized']['times'][i] 
                            for i in range(len(self.test_sizes)) if self.results['optimized']['times'][i] > 0])
...
def main():
    """主函数"""
    print("日记推荐算法增强版性能对比测试")
    print("="*60)
    
    # 创建测试实例
    test = EnhancedDiaryRecommendationTest()
