...
class AIGC:
    ...
    def generate_video_complete_workflow(self, image_path: str, custom_prompt: Optional[str] = None, 
        ...
        Args:
            ...
            save_path: 视频保存路径（可选，可以是文件路径或目录路径）
        ...
        try:
            ...
            if save_path is None:
                ...
                # 检查save_path是目录还是文件
                if os.path.isdir(save_path) or (not os.path.exists(save_path) and not save_path.endswith('.mp4')):
                    # 如果是目录或者路径不存在且不以.mp4结尾，则视为目录
                    timestamp = int(time.time())
                    save_path = os.path.join(save_path, f"generated_video_{timestamp}.mp4")
    ...
    def _download_video(self, video_url: str, save_path: str) -> str:
        ...
        Args:
            video_url: 视频URL
            save_path: 本地保存路径（必须是完整的文件路径，不能是目录）
        ...
        try:
            ...
            if not save_path.endswith('.mp4'):
                # 如果不以.mp4结尾，可能是目录路径，这是错误的
                raise ValueError(f"save_path必须是完整的文件路径，不能是目录: {save_path}")
            ...
            if save_dir:  # 只有当目录不为空时才创建
                try:
                    ...
                    raise PermissionError(f"无法创建保存目录 {save_dir}: {e}")
            ...
            try:
                # 尝试创建一个临时文件来测试写入权限
                test_file = save_path + ".tmp"
                with open(test_file, 'w') as f:
                    f.write("test")
                os.remove(test_file)
            except PermissionError as e:
                raise PermissionError(f"没有权限写入文件 {save_path}: {e}")
